...
- **Saída JSON tolerante (desde 19/09/2026):** os chain nodes NÃO usam mais o *Structured Output Parser* — ele derrubava a chain inteira com "Model output doesn't fit required format" à menor sujeira na saída (cerca ```json, raciocínio antes do JSON, vírgula sobrando), o que desperdiçava provedores da cascata (os 3 Groq nunca serviam de fallback). Agora o esquema JSON está escrito no próprio prompt (bloco "FORMATO DE SAÍDA") e, depois de cada chain, um nó **Code** (`JSON <nome da chain>`) extrai o JSON de forma tolerante e valida as chaves obrigatórias do fluxo (ex.: `casos`, `matriz`, `casos_de_teste`, `planos_de_teste`, `vinculos`, `comparacoes`, `wiql`+`titulo_sugerido`+`explicacao`, `contexto`+`escopo_proposito`+`conclusao`, `titulo_manual`+`introducao`+`passos`, `duvidas`). Se ainda assim não der, o erro sai pela saída de erro do Code e segue pro próximo provedor — o mesmo roteamento de antes. Modelos Groq com temperatura 0,2 e limite de 6.000 tokens de saída. Único fluxo sem isso: `Doc_QA_Image_Interpretation` (saída em texto, sem parser).
...
- **Rate limit por lote**: um único lote de geração já pode consumir a maior parte da cota por minuto (TPM) de um provedor — erro real capturado no Groq: `Limit 8000, Used 2331, Requested 6285` (~78% do limite numa chamada só). Por isso, quando uma chamada falha, o app espera ~62s (alinhado à janela real do rate limit) antes de tentar a mesma de novo (até 3x) e antes da seguinte; entre unidades que deram certo o intervalo é de só 5s nos lotes de geração e nenhum nas demais (`_ESPERA_ENTRE_LOTES_SEGUNDOS` / `_ESPERA_APOS_ERRO_SEGUNDOS` / `_MAX_TENTATIVAS_POR_LOTE` em `ui/ia_retry.py`), já que o fallback de seis provedores no n8n costuma absorver a carga. A regra é a mesma pra todas as chamadas de IA: `_processar_um_lote_por_execucao` (lotes e imagens) e `_chamar_ia_com_retentativas` (chamada única — Análise, Vínculos, Narrativa, WIQL, Manual, Testes de API); a espera nunca trava uma execução do Streamlit (reruns de ~2s), e a preparação cara de cada fluxo (extrair documento, buscar Work Items) fica em cache no estado (`_<prefixo>_prep`) pra não se repetir a cada rerun. Só a comparação de duplicados do Passo 7 fica de fora (falha não bloqueia).
...
│   │   ├── api_bugs_page.py            # Testes de API — etapa 3: análise, rascunhos de Bug, envio ao Azure (mixin)
...
│   │   ├── pdf_report.py               # PDF (Documentação QA e Relatório de Testes) — `_esc` troca caracteres sem glifo na fonte (‑ ≤ → …) por equivalentes; `_nova_pagina` evita página em branco
...
│   │   ├── api_triage.py               # triagem do resultado + achados (é bug? o que fazer, com quem falar, o que dizer) + rascunho de Bug
│   │   ├── api_contracts.py            # catálogo de formatos reais por host + sugestões de correção de formato
│   │   ├── api_autofill.py             # login de outros perfis, dados negativos, ids descobertos nas respostas
│   │   ├── api_generation_batches.py   # geração com IA em lotes de Work Items (contexto entre lotes, dedupe)
...
└── docs/
    ...
    └── Guia_Testes_API.pdf / .docx     # manual técnico dos Testes de API, para desenvolvedores (PDF copiado em qa_testgen/assets, botão na tela)
```
...
|---|---|
| 1. Definição | Nome, Ambiente, Base URL (`{{base_url}}`), origem dos casos (geração por IA via `Doc_QA_ApiTest_Generation` a partir de Work Item(s) do Azure DevOps — `_setup_azure_devops_connection` + board por Area Path, Description + Acceptance Criteria — ou texto colado/documentos; `api_discovery.py` ("Reconhecer a API") sonda as rotas citadas — variantes /api → /api/v1 — sem credencial, pelo mesmo caminho da bateria, e gera as Observações (rota real, `message` i18n, `errors.<campo>`, rotas 401); **catálogo de rotas reais** por host (`app_config` chave `api_rotas::<host>`, cache em `api_catalogo`): `extrair_rotas_de_bundle` lê os `.js` do front (`descobrir_bundles` acha os scripts da página da Base URL; sob WAF, upload do `.js`), `extrair_rotas_de_openapi`/`_postman`/`_texto`; `rotas_para_prompt` injeta o catálogo nas Observações da geração ("SOMENTE estas rotas"), `_api_aplicar_catalogo` desabilita casos fora dele, `montar_sondas_para_casos` + `verificar_respostas_das_sondas` ("Verificar rotas dos casos") confirmam na API (`rota_nao_encontrada`: 404 "route could not be found" ou 404 sem JSON), execuções e sondagens alimentam o catálogo; painel `_api_prontidao` lista o que falta pra gerar com precisão; os Work Items escolhidos ficam em `api_work_items` para o vínculo da Fase 2; collection Postman + environment opcional; definição `.json` do próprio módulo; criação manual), variáveis (secretas só em sessão), documentos de contexto opcionais, editor por caso (método, URL, headers, body, asserções declarativas, extração de variáveis) |
...
| 3. Análise e Bugs | `ui/api_bugs_page.py` (mixin `ApiAnaliseBugsMixin`). `api_triage.analisar(casos_executados, resultados, contexto)` classifica cada resultado não aprovado em `bug_api`, `perfil`, `a_confirmar`, `bateria_auth`, `bateria_requisicao`, `bateria_formato`, `sintaxe`, `desempenho`, `rota_inexistente`, `sem_resposta`, `bloqueado` e levanta **achados** (`vazamento_interno`, `erro_servidor`, `acesso_indevido`, `validacao_ausente`, `dado_sensivel`, `lentidao`) com `analise`, `como_confirmar`, `o_que_fazer`, `com_quem_falar`, `mensagens`. "Requisito" = status/mensagem citados no texto dos Work Items (`contexto.especificacao`); sem texto, divergência vai pro PO. `contexto.responsaveis` (de `AzureDevOpsClient.get_work_items_status`, AssignedTo) troca o papel pelo nome. Rascunhos (`api_bug_rascunhos`) com ver/editar/excluir; envio reaproveita `_setup_azure_devops_connection`, `_render_bug_metadata_picker(…, 'api', …)` e `_criar_bug_via_azure`, e depois `upload_attachment`/`attach_file_to_work_item` (request/response `.txt` mascarados via `api_triage.arquivos_de_evidencia` + PDF), `link_bug_to_related_work_item` (Work Items da geração) e `link_bug_to_test_case` (via `ado_test_case_ids`). Confirmação inline → `_api_bug_envio_confirmado` → `current_action='api_enviar_bugs'` (overlay) → `api_bug_envio_resultado` (link por Bug + `_wi_fila_url_consulta`). `api_contracts.sugerir_correcoes/aplicar_correcoes`: só formato (header Authorization, renomear campo pelo 422, caminho real, extração de id, circular→não vazio; frases guardadas em `api_correcoes_aplicadas`). **Relatório antes do Azure** (`_api_render_relatorio_previo`): gera o mesmo PDF/MD da etapa 4 com `api_triage.linhas_relatorio/markdown_da_analise(analise, rascunhos, casos, correcoes, segredos)` — todos os Bugs, rascunho ou enviado, mascarados; `api_relatorio_fp` + `_api_relatorio_desatualizado()` avisam se rascunhos/correções/responsáveis mudaram depois |
| 4. Evidências | `ApiEvidenceBuilder` gera `RELATORIO.md` e `.zip` (pasta por caso: `1_request.txt`, `2_response.txt`, `3_resultado.txt` + imagens); `PdfReportGenerator.generate_api_test_report` gera o PDF no padrão do app; seção "Análise automática" (`api_triage.markdown_da_analise` / `linhas_relatorio`) com os Bugs abertos; opção de salvar no Documentos Armazenados (tipos `pdf`, `md`, `zip`) |
...
- Testes unitários em `tests/test_api_tests_module.py` (importador, runner com servidor HTTP local, avaliação de respostas externas, reconhecimento da API, mascaramento/zip, subconjunto JSONPath), `tests/test_api_triage.py` (triagem, achados, rascunho mascarado, catálogo de formatos, correções que nunca mudam status), `tests/test_api_autofill.py` e `tests/test_api_generation_batches.py`.
- **Catálogo de formatos reais** (`app_config` chave `api_contratos::<host>`, cache `api_contratos`): `api_contracts.aprender` roda em `_api_concluir_execucao` (junto com a foto `api_casos_executados`, `api_execucao_id`, `api_executado_em`); `api_contracts.para_prompt` entra nas Observações da geração (chamada única e lotes), depois do bloco de rotas.
- **Caminho JSON** (`ApiTestRunner.obter_caminho` e `obterCaminho` do componente do navegador, mesma gramática): `a.b`, `a[0]`, `a[-1]`, `a[*]`, `a.*`, `a['chave']`, `a[?(@.c=='x')]`, `a[?(@.c==7)]`, `a[?(@.c)]`, `a.length`, prefixo `$`. `[*]`/filtro devolvem `Varios` (asserção passa se UM valor atende; `json_type` exige todos); sintaxe fora disso levanta `CaminhoInvalido` e a asserção diz "sintaxe de caminho não suportada" (nunca "campo ausente").
- **Variável sem valor** não trava a etapa: `motivo_bloqueio` (Python) e o componente (JS) pulam só os casos que a usam; `_api_avisos_variaveis` avisa antes de executar.
...
**Azure DevOps:** `api_to_assistant.converter_bateria` transforma casos + resultados em `matriz`/`test_cases`/`test_plans` da sessão (suíte e funcionalidade da Matriz agrupadas por `_rota` = método + `normalizar_caminho` do template: `{{var}}`/id → `{id}`, sem query; o passo guarda a URL concreta; "Última execução" com o horário da execução — `executado_em` = `api_executado_em` —, nunca o da conversão; categoria Negativo também pelo nome do caso, ex. "fora do escopo (200 sem dados)") (formato idêntico ao do assistente; `origem: testes_api`, `api_case_id`, `work_item_relacionado` = Work Item da geração) e o app navega pro Passo 5; o Passo 7 segue inalterado. Após o push, `ado_last_plan_id` + `ado_test_case_ids` + `api_test_run_pendente` habilitam "Registrar Test Run": `list_plan_suites` → `list_test_points` → `create_test_run` → `update_test_run_results` (Passed/Failed + comentário) → `attach_file_to_test_run` (PDF) → `complete_test_run`.

**Criar Work Item — Fila / planilha:** `infrastructure/work_item_batch.py` (modelo XLSX/CSV gerado do projeto, leitura CSV/XLSX/TXT, validação linha a linha com mensagens que dizem a coluna e o valor esperado, `problemas_do_arquivo` para colunas obrigatórias ausentes, `resumo_pendencias`, ordenação pai→filho por `#Ref`) e `ui/work_item_batch_page.py` (mixin: fila, upload com diagnóstico `_wi_fila_mostrar_diagnostico`, envio com confirmação, resumo `_wi_fila_resultados` com links diretos e consulta WIQL temporária `_queries/query/?wiql=`; o rádio de modo usa rótulos fixos porque rótulo dinâmico faz o Streamlit recriar o widget e voltar à primeira opção). `openpyxl` no requirements.
